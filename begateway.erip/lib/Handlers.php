...
class Handlers
{
  ...
  static public function setEripOrderAutomatic($id, $status)
  {
    try
    {
      ...
      if (self::$values["PAY_SYSTEM_ID"] != self::$opt_payment)
        throw new \Exception("ERIPAUTOMODE10-" . self::$values["PAY_SYSTEM_ID"] . '-' . self::$opt_payment);

      if ($status != self::$opt_status)
        throw new \Exception("ERIPAUTOMODE12-{$status}-" . self::$opt_status);

      self::set_and_send();

      $order_update = CSaleOrder::Update($id, array("COMMENTS" => "status: ". self::$o_response->transaction->status ."\n".
        "transaction_id: ". self::$o_response->transaction->transaction_id ."\n".
        "order_id: ". self::$o_response->transaction->order_id ."\n".
        "account_number: ". self::$o_response->transaction->erip->account_number ."\n"));
